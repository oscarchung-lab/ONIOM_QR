...
program gau2xtb
    ...
    if(narg == 6) then
        ...
        call system('rm -f charges energy xtbrestart gradient hessian')
        call system('rm -f hessian xtb_normalmodes g98_canmode.out g98.out wbo xtbhess.coord')
    !add solvation option
    elseif(narg == 7) then
        call getarg(1, solvation)
        ...
        if (deriva == 0) then
            commandline='xtb gau_xtb.xyz --chrg '//trim(adjustl(chrg))//' --uhf '//trim(adjustl(uhf))//' --sp '//'--gbsa '//trim(solvation)//' > gau_xtb.out' 
        elseif (deriva == 1) then
            commandline='xtb gau_xtb.xyz --chrg '//trim(adjustl(chrg))//' --uhf '//trim(adjustl(uhf))//' --grad '//'--gbsa '//trim(solvation)//' > gau_xtb.out'          
        elseif (deriva == 2) then
            commandline='xtb gau_xtb.xyz --chrg '//trim(adjustl(chrg))//' --uhf '//trim(adjustl(uhf))//' --hess --grad '//'--gbsa '//trim(solvation)//' > gau_xtb.out'
        endif
